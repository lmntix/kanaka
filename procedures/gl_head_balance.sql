...
-- Create date: 24/12/2008
-- Description:	To calculate Balance as per sub_accounts_id only for sub_accounts_no = 0
-- =============================================
create PROCEDURE [dbo].[sp_GLHead_Balance](@strAccountList varchar(8000),@dteToDate datetime,@strUserId varchar(300),@Delimiter char(1) = ',')
...
Else
 begin
      DECLARE rt_cursor CURSOR FOR 
     (SELECT sub_accounts_id FROM sub_accounts  WHERE accounts_id = @Item AND sub_accounts_no =0)
 end
